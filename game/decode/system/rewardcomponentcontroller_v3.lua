...
function RewardController:RegisterRewards()
  ...
  if self.ItemDB ~= nil and self.RewardEvent ~= nil then
    if LuaLibCore:VerifyDataInputEntity(self.entity, "entity") then
      RegisterReward_V2(self.entity, self.ItemDB, self:GetRewardEventEnumValue(self.RewardEvent), self.missionId)
    end
    self:RewardsRegistered()
  end
  if self.ItemDBList ~= nil and self.RewardEvent ~= nil then
    if LuaLibCore:VerifyDataInputEntity(self.entity, "entity") then
      RegisterRewardList_V2(self.entity, self.ItemDBList, self:GetRewardEventEnumValue(self.RewardEvent), self.missionId)
    end
    self:RewardsRegistered()
  end
  self:Out()
...
function RewardController:ExecuteRewards()
  ...
  if self.ItemDB ~= nil then
    ExecuteReward_V3(self.pawn, self.ItemDB, self.DisplayFeedback, self.missionId)
    self:RewardsExecuted()
  end
  if self.ItemDBList ~= nil then
    ExecuteRewardList_V3(self.pawn, self.ItemDBList, self.DisplayFeedback, self.missionId)
    self:RewardsExecuted()
  end
  self:Out()
end
function RewardController:GetRewardEventEnumValue(rewardEvent)
  ...
  elseif rewardEvent == "OnPickedItem" then
    return 4
  elseif rewardEvent == "GenericInteraction" then
    return 13
  end
  return -1
end
