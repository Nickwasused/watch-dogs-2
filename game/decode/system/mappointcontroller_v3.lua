...
function MapPointController:Show()
  ...
  if self.MapPoint ~= nil then
    SendMapPointControllerEvent(self.MapPoint, 1)
    ...
    if self.Player ~= nil then
      ...
      if self.LocText ~= nil then
        if tonumber(self.LocText.id) ~= nil then
          IndicatorSetLocNameId(self.Player, self.MapPoint, self.LocText.id)
        end
      elseif self.Text ~= nil then
        IndicatorSetName(self.Player, self.MapPoint, self.Text)
      end
    end
  end
  Shown(self)
...
function MapPointController:Hide()
  ...
  if self.MapPoint ~= nil then
    SendMapPointControllerEvent(self.MapPoint, 0)
    if self.Player == nil then
      self.Player = GetLocalPlayerId()
    end
    if self.Player ~= nil then
...
function MapPointController:HideText()
  ...
  if self.MapPoint ~= nil then
    ...
    if self.Player ~= nil then
      IndicatorSetName(self.Player, self.MapPoint, "")
    end
  end
  TextHidden(self)
end
